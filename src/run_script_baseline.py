...
for config in ['partial_sorting_4_config_cotrain']:
    ...
    for data_ratio in data_ratios:
        ...
        for seed in seeds:
            ...
            agent_name = f'3781_idm_baseline_{backbone}_{data_ratio}_{seed}'
            command1 = (f'python run.py --config-name={config} agents=idm_baseline agent_name={agent_name} '
                        f'action_data_ratio={data_ratio} window_size={window_size} seed={seed} hidden_dims={hidden_dims} device={device} '
                        f'backbone={backbone} trainset.only_label_data=True trainset.obs_data_ratio={data_ratio}')
        ...
        for seed in seeds:
            ...
            command2 = (
                f'python run_sim_baseline.py --config-name={config} agents=idm_agent_baseline agent_name={agent_name} '
                f'action_data_ratio={data_ratio} window_size={window_size} seed={seed} hidden_dims={hidden_dims} device={device} '
                f'backbone={backbone} trainset.only_label_data=True  trainset.obs_data_ratio={data_ratio} '
