...
class CustomDataset(torch.utils.data.Dataset):
    def __init__(self, data, expected_shape):
        for item in data:
            for elem in item:
                print(elem.shape)
            break
        self.data = [item for item in data if item[0].shape[0] == expected_shape]
...
class BaseAgent(abc.ABC):
    ...
    def __init__(
        ...
        if finetuneset is not None:
            print('init finetune dateset')
            self.finetuneset = hydra.utils.instantiate(finetuneset)
            self.tune_dataloader = torch.utils.data.DataLoader(
                self.finetuneset,
                batch_size=train_batch_size,
                shuffle=True,
                num_workers=num_workers,
