...
class Stacking_Sim(BaseSim):
    def __init__(
        ...
        # Update modes to use only red and green
        self.mode_1 = {'r': 0, 'g': 1}
        self.mode_2 = {'rg': 0, 'gr': 1}

        self.modes = np.ones(2) / 2

        self.mode_encoding_2 = np.zeros(2)

        for m_key, i in zip(self.mode_2.keys(), range(2)):
            self.mode_encoding_2[self.mode_2[m_key]] = self.modes[i]

        self.mode_encoding_1 = np.array([self.mode_encoding_2[i] for i in [0, 1]])

        self.mode_encoding_1 = torch.tensor(self.mode_encoding_1)
        self.mode_encoding_2 = torch.tensor(self.mode_encoding_2)


    def eval_agent(self, agent, contexts, n_trajectories, mode_encoding, mode_encoding_1_box,
                   successes, successes_1, cpu_set, pid):
        ...
        valid_modes = set(self.mode_1.keys()).union(set(self.mode_2.keys()))
        ...
        for context in contexts:
            ...
            for i in range(n_trajectories):
                ...
                if len(info['mode']) > 1 and mode[:2] in valid_modes:
                    mode_encoding_1_box[context, i] = torch.tensor(self.mode_1[mode[:1]])
                    mode_encoding[context, i] = torch.tensor(self.mode_2[mode[:2]])

                elif len(info['mode']) > 0 and mode[:1] in valid_modes:
                    mode_encoding_1_box[context, i] = torch.tensor(self.mode_1[mode[:1]])
                ...
                suc = info['success']
                suc_1 = info['success_1']
                print(f'Context {context} Rollout {i} Success {suc} Success_1 {suc_1}')

    def cal_KL(self, mode_encoding, successes, prior_encoding, n_mode=2):
    ...
    def test_agent(self, agent):
        ...
        if self.n_cores > 1:
            ...
            self.eval_agent(agent, contexts, self.n_trajectories_per_context, mode_encoding, mode_encoding_1_box,
                            successes, successes_1, set([0]), 0)

        box1_success_rate = torch.mean(successes_1).item()
        success_rate = torch.mean(successes).item()

        entropy_1, KL_1 = self.cal_KL(mode_encoding_1_box, successes_1, self.mode_encoding_1, n_mode=2)
        entropy_2, KL_2 = self.cal_KL(mode_encoding, successes, self.mode_encoding_2, n_mode=2)

        wandb.log({'score': (box1_success_rate + success_rate)})
        ...
        log.info(f'entropy_2: {entropy_2}')
        log.info(f'KL: {KL_2}')

        log.info(f'successes_1_box: {box1_success_rate}')
        log.info(f'entropy_1: {entropy_1}')
        log.info(f'KL: {KL_1}')

        return (success_rate, box1_success_rate)
