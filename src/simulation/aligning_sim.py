...
class Aligning_Sim(BaseSim):
        ...
        for context in contexts:
            for i in range(n_trajectories):
                ...
                if self.if_vision:
                    ...
                    #
                    #     obs, reward, done, info = env.step(pred_action)
                    # print(f"pred_action:{pred_action[:3]}, obs:{obs}")
                    obs = np.concatenate((pred_action[:3], obs))
                    ...
                    while not done:
                        ...
                        pred_action_list = agent.predict(obs_seq)

                        # for k in range(len(pred_action_list)):
                        for k in range(4):

                            pred_action = pred_action_list[k] + obs[:3]
                            pred_action = np.concatenate((pred_action, [0, 1, 0, 0]), axis=0)
    ...
    def test_agent(self, agent):
        ...
        log.info(f'Mean Distance {mean_distance.mean().item()}')
