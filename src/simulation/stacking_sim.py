...
class Stacking_Sim(BaseSim):
    def __init__(
        ...
        self.modes = np.ones(6)/6

        self.mode_encoding_3 = np.zeros(6)
        self.mode_encoding_2 = np.zeros(6)

        for m_key,i in zip(self.mode_3.keys(),range(6)):

            self.mode_encoding_3[self.mode_3[m_key]] = self.modes[i]
            self.mode_encoding_2[self.mode_3[m_key]] = self.modes[i]
        ...
        # print(self.mode_encoding_1,self.mode_encoding_2,self.mode_encoding_3)
    ...
    def eval_agent(self, agent, contexts, n_trajectories, mode_encoding, mode_encoding_1_box, mode_encoding_2_box,
        ...
        for context in contexts:
            ...
            for i in range(n_trajectories):
                ...
                suc_2 = info['success_2']
                print(f'Context {context} Rollout {i} Success {suc} Success_1 {suc_1} Success_2 {suc_2}')
    ...
    def test_agent(self, agent):
        ...
        log.info(f'entropy_3: {entropy_3}')
        log.info(f'KL: {KL_3}')

        log.info(f'successes_1_box: {box1_success_rate}')
        log.info(f'entropy_3: {entropy_1}')
        log.info(f'KL: {KL_1}')

        log.info(f'successes_2_boxes: {box2_success_rate}')
        log.info(f'entropy_3: {entropy_2}')
        log.info(f'KL: {KL_2}')


        return (success_rate,box1_success_rate,box2_success_rate)
